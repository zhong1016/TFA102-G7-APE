...
public class ManagerJDBCDAO   implements  ManagerDAO_interface{
	...
	String password = "123456";
			...
			private static final String INSERT_STMT = 
					"INSERT INTO manager(account,password,nickname,phone,email,headshot)  values (?,?,?,?,?,?)";
			private static final String UPDATE=
					"UPDATE manager set account=?,  password=?,  nickname=?, phone=?, email=? where manager_no =?";
			private static final String DELETE =
					"DELETE FROM manager where manager_no = ?";
			private static final String GET_ONE_STMT = 
					"SELECT * FROM manager where manager_no = ?";
			private static final String GET_ALL_STMT =
					"SELECT  * FROM manager";
			private static final String GET_BY_ACCOUNT =
					"SELECT * FROM manager where account = ?";
			//更新管理員資料(含頭貼)	
			private static final String UPDATE_INCLUDE_PIC =
					"UPDATE MANAGER set headshot = ?,nickname = ?, phone = ?, email =  ? where manager_no = ?";;
			
					
			//更新管理員資料(不含頭貼)		
			private static final String UPDATE_EXCLUDE_PIC =
					"UPDATE MANAGER set nickname = ?, phone = ?, email =  ? where manager_no = ?";
