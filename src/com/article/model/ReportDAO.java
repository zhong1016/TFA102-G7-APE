...
public class ReportDAO implements Report_interface {
	...
	private static final String INSERT_STMT = 
			"INSERT INTO REPORT (REPORT,REPORT_MSG,REPORT_STATUS, MANAGER_NO , MSG_NO) VALUES ( ?, ?, ?, ? , ?)";
	private static final String GET_ALL_STMT = 
			"SELECT REPORT_NO,REPORT,REPORT_MSG,REPORT_STATUS,MANAGER_NO FROM REPORT order by REPORT_NO";
	private static final String GET_ONE_STMT = 
			"SELECT REPORT_NO,REPORT,REPORT_MSG,REPORT_STATUS,MANAGER_NO,MSG_NO FROM REPORT where REPORT_NO = ?";
	...
	public void insert(ReportVO reportVO) {
		...
		try {
			...
			pstmt.setInt(5, reportVO.getMSG_NO());
			pstmt.executeUpdate();
	...
	public ReportVO findPrimaryKey(Integer REPORT_NO) {
		...
		try {
			...
			while(rs.next()) {
				...
				reportVO.setMSG_NO(rs.getInt("MSG_NO"));
			}
